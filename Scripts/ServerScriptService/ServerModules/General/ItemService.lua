...
local function RunSpawning()
    task.spawn(function()
        while true do
            ...
            ItemService:SpawnCrate(SpawnHere, ItemInfo.NormalCrate)
        end
    end)
end
...
function ItemService:SpawnCrate(CF: CFrame, Crate: CustomEnum.Crate, Pickups: {{PickupType: CustomEnum.Pickup, Amount: number | NumberRange}}?, Health: number?)
